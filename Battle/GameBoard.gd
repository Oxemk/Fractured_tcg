...
func _load_config() -> void:
	config["mode"] = "pvp" if game_mode == GameMode.PVP else "pve"
...
func _start_new_turn() -> void:
	...
	if game_mode == GameMode.PVP:
		current_turn_owner = TurnOwner.PLAYER1 if turn_count % 2 == 1 else TurnOwner.PLAYER2
	else:
		current_turn_owner = TurnOwner.PLAYER1 if turn_count % 2 == 1 else TurnOwner.AI
...
func switch_to_camera(target: String) -> void:
	match target:
		"p1": cam_p1.make_current()
		"p2": cam_p2.make_current()
		_: cam_center.make_current()
...
func draw_cards(count: int, is_player1: bool) -> void:
	var deck = p1_deck if is_player1 else (p2_deck if game_mode == GameMode.PVP else ai_deck)
	var hand = PlayerHand if is_player1 else EnemyHand
...
func populate_hand(deck: Array, hand: Node2D) -> void:
	...
	for i in range(deck.size()):
		...
		if node:
			...
			hand.add_child(node)
			node.position = Vector2(i * spacing, 0)
...
func build_ai_deck(_cfg: Dictionary) -> Array:
	...
	return ["char_001", "weap_001", "armor_1", "trap_1", "support_1"]
